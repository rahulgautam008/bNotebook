...
const JWT_SECRET = "rahulisagood&vboy";
   ...
   let success = false;
    ...
    try {
      ...
      if (!user) {
        ...
        return res
          .status(400)
          .json({ error: "Please try login with correct critdentails" });
      }
      ...
      if (!passwordCompare) {
        ...
        return res
          .status(400)
          .json({success, error: "Please try login with correct critdentails" });
      }
...
// route 3  : get logged user details  using: POST "/api/auth/geteuser". Login required
